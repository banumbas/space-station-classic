...
using Content.Server._Starlight.Antags.Cult;
...
public sealed class CultRuleSystem : GameRuleSystem<CultRuleComponent>
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<CultPortalLifeTimeEvent>(PortalLifeTimeEvent);

        SubscribeLocalEvent<CultRoleComponent, GetBriefingEvent>(OnGetBriefing);
        SubscribeLocalEvent<CultLeaderRoleComponent, GetBriefingEvent>(OnGetBriefing);
        ...
        //SubscribeLocalEvent<CultRuleComponent, AfterAntagEntitySelectedEvent>(OnAfterAntagEntSelected);
        //    SubscribeLocalEvent<NukeopsRuleComponent, RuleLoadedGridsEvent>(OnRuleLoadedGrids);
    }

    private void PortalLifeTimeEvent(ref CultPortalLifeTimeEvent ev)
    {
        var query = QueryActiveRules();
        while (query.MoveNext(out var uid, out _, out var cult, out _))
        {
            var duration = Timing.CurTime.Subtract(cult.StartTime);
            if (duration > cult.WhenPortalPermanent)
            {
                ev.LifeTime = null;
                return;
            }

            var lifetime = duration.TotalSeconds * cult.PortalLifetimeMultiplier;

            ev.LifeTime = (float)Math.Max(lifetime, cult.PortalMinTime.TotalSeconds);
            return;
        }
    }

    protected override void Started(EntityUid uid,
        CultRuleComponent component,
    ...
    //{
    //    switch (component.WinType)
    //    {
    //        case CultWinType.God:
    //            args.AddLine(Loc.GetString(component.GodReleasedRoundEndText));
    //            break;
    //        case CultWinType.Crew:
    //            args.AddLine(Loc.GetString(component.GodDeadRoundEndText));
    //            break;
    //        default:
    //            break;
    //    }

    //    args.AddLine(Loc.GetString(component.AntagList));

    //    var antags =_antag.GetAntagIdentifiers(uid);

    //    foreach (var (_, sessionData, name) in antags)
    //    {
    //        args.AddLine(Loc.GetString(component.AntagListNameUser, ("name", name), ("user", sessionData.UserName)));
    //    }
    //}
    ...
    private void OnNukeExploded(NukeExplodedEvent ev)
    {
        ...
        while (query.MoveNext(out var uid, out _, out var cult, out _))
        {
            map ??= _map.GetMap(ev.OwningMap);

            if (TryComp<CultistMapComponent>(map, out _))
                SetWin((uid, cult), CultWinType.Crew);
            else SetWin((uid, cult), CultWinType.Tie);
        }
    }
    ...
    private void OnShuttleCallAttempt(ref CommunicationConsoleCallShuttleAttemptEvent ev)
    {
        ...
        {
            if (Timing.CurTime.Subtract(cult.StartTime) < cult.EvacShuttleDisabled)
            {
                ev.Cancelled = true;
                ev.Reason = Loc.GetString("cult-shuttle-call-unavailable");
                return;
            }
        }
    }
    ...
    private void SetWin(Entity<CultRuleComponent> ent, CultWinType type)
    {
        ent.Comp.WinType = type;

        if (type == CultWinType.God)
            _roundEndSystem.DoRoundEndBehavior(ent.Comp.GodReleasedRoundEndBehavior,
                ent.Comp.RoundEndDelay,
                ent.Comp.GodReleasedRoundEndTextSender,
                ent.Comp.GodReleasedRoundEndTextShuttleCall,
                ent.Comp.GodReleasedRoundEndText);
        ;
    }
    ...
    //private void OnAfterAntagEntSelected(Entity<CultRuleComponent> ent, ref AfterAntagEntitySelectedEvent args)
    //{
    ...
    private void OnGetBriefing(Entity<CultLeaderRoleComponent> ent, ref GetBriefingEvent args)
        => args.Append(Loc.GetString(ent.Comp.Briefing));

    private void OnGetBriefing(Entity<CultRoleComponent> ent, ref GetBriefingEvent args)
        => args.Append(Loc.GetString(ent.Comp.Briefing));
...
[ByRefEvent]
public struct CultPortalLifeTimeEvent
{
