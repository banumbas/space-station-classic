...
/// <summary>
/// Time-sliced wrapper around <see cref="ClassicStabilityFloodFill"/> for one whole Z-column — every grid
/// bridged to every other via a chain of <see cref="ClassicGridStabilitySupportComponent"/>s, computed
...
/// queued (see <see cref="ClassicZCollapseSystem"/>) — <see cref="Process"/> never touches
    ...
    : Job<Dictionary<(EntityUid Grid, Vector2i Tile), int>>(maxTime, cancellation)
{
    ...
    protected override async Task<Dictionary<(EntityUid Grid, Vector2i Tile), int>?> Process()
    {
        ...
        ClassicStabilityFloodFill.SeedCores(stability, queue, _liveNodes, coreSeeds);

        while (ClassicStabilityFloodFill.Process(queue, stability, _liveNodes, bridges, BatchSize) > 0)
        {
