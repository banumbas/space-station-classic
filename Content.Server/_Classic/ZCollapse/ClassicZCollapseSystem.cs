...
using Content.Server._Classic.ZLevels.Gravity;
...
public sealed partial class ClassicZCollapseSystem : EntitySystem
{
    ...
    [Dependency] private EntityQuery<ClassicGridStabilityComponent> _stabilityQuery = default!;
    [Dependency] private EntityQuery<ClassicGridStabilityCoreComponent> _coreQuery = default!;
    [Dependency] private EntityQuery<ClassicGridStabilitySupportComponent> _supportQuery = default!;
    [Dependency] private EntityQuery<MapGridComponent> _gridQuery = default!;
    [Dependency] private EntityQuery<ClassicZMapComponent> _zMapQuery = default!;
    [Dependency] private EntityQuery<ClassicZMapNetworkComponent> _zNetworkQuery = default!;
    ...
    /// </summary>
    private void RescanGridIndex(EntityUid gridUid, ClassicGridStabilityComponent comp)
    {
        comp.Cores.Clear();
        comp.Supports.Clear();

        var coreQuery = AllEntityQuery<ClassicGridStabilityCoreComponent, TransformComponent>();
        ...
        var supportQuery = AllEntityQuery<ClassicGridStabilitySupportComponent, TransformComponent>();
    ...
    /// "has <see cref="ClassicGridStabilityComponent"/>"; the mapping-preview path (<see cref="AddPreviewSnapshots"/>)
    ...
    private List<EntityUid> GetColumn(EntityUid startGrid, Func<EntityUid, bool> participates)
    {
        ...
        // ClassicGridStabilityComponent), the mapping-preview path passes IsZCollapseEligible instead
    ...
    /// snapshot is O(entities in this column) via each grid's <see cref="ClassicGridStabilityComponent.Cores"/>/
    /// <see cref="ClassicGridStabilityComponent.Supports"/> index — never a world-wide scan.
    /// </summary>
    ...
    /// <see cref="ClassicGridStabilityComponent.Stability"/>. No cross-grid cascade step is needed here —
    ...
    /// </summary>
    private void ScheduleCollapsingTiles(EntityUid gridUid, MapGridComponent grid, ClassicGridStabilityComponent comp, IReadOnlySet<Vector2i> liveTiles, Dictionary<Vector2i, int> newStability)
    {
    ...
    private void ProcessPendingCollapses()
    {
        ...
        var query = AllEntityQuery<ClassicGridStabilityComponent, MapGridComponent>();
