...
public sealed partial class ClassicZLevelsLaddersCacheSystem : EntitySystem
{
    ...
    [Dependency] private EntityQuery<ClassicZLevelsLaddersCacheComponent> _cacheQuery = default!;

    public override void Initialize()
    {
        base.Initialize();

        SubscribeLocalEvent<ClassicZLevelHighGroundComponent, MapInitEvent>(OnLadderInit);
        SubscribeLocalEvent<ClassicZLevelHighGroundComponent, ComponentShutdown>(OnLadderShutdown);
        SubscribeLocalEvent<ClassicZLevelHighGroundComponent, AnchorStateChangedEvent>(OnLadderAnchorChanged);
    }

    private void OnLadderInit(Entity<ClassicZLevelHighGroundComponent> ent, ref MapInitEvent args)
    {
        TryCacheLadder(ent);
    }

    private void OnLadderShutdown(Entity<ClassicZLevelHighGroundComponent> ent, ref ComponentShutdown args)
    {
        TryRemoveCache(ent);
    }

    private void OnLadderAnchorChanged(Entity<ClassicZLevelHighGroundComponent> ent, ref AnchorStateChangedEvent args)
    {
        if (args.Anchored)
            TryCacheLadder(ent);
        else
    ...
    /// </summary>
    private void TryCacheLadder(Entity<ClassicZLevelHighGroundComponent> ent)
    {
        ...
        var cache = EnsureComp<ClassicZLevelsLaddersCacheComponent>(gridUid.Value);
        ...
        cache.Slopes[tilePos] = new ClassicCachedSlope
        {
            Entity = ent,
            Direction = dir,
        };
    ...
    /// </summary>
    private void TryRemoveCache(Entity<ClassicZLevelHighGroundComponent> ent)
    {
        if (!_xformQuery.TryGetComponent(ent, out var xform))
            return;
