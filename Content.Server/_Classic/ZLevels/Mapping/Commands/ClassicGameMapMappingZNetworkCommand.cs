...
public sealed partial class ClassicGameMapMappingZNetworkCommand : LocalizedEntityCommands
{
    [Dependency] private IPrototypeManager _proto = default!;
    [Dependency] private IComponentFactory _compFactory = default!;
    [Dependency] private MapLoaderSystem _mapLoader = default!;
    [Dependency] private ClassicZLevelsSystem _zLevel = default!;
    ...
    public override CompletionResult GetCompletion(IConsoleShell shell, string[] args)
    {
        ...
        {
            // Check if any station in this map has ClassicStationZLevelsComponent  component with maps
            var hasZLevels = false;
            foreach (var station in map.Stations.Values)
            {
                if (station.StationComponentOverrides.TryGetComponent<ClassicStationZLevelsComponent>(_compFactory, out var zNetwork))
                {
        ...
        return CompletionResult.FromHintOptions(options, "GameMapPrototype with ClassicStationZLevelsComponent");
    }
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        // Find the first station with ClassicStationZLevelsComponent
        ClassicStationZLevelsComponent? zNetwork = null;
        foreach (var station in mapProto.Stations.Values)
        {
            if (station.StationComponentOverrides.TryGetComponent<ClassicStationZLevelsComponent>(_compFactory, out var zNet))
            {
                zNetwork = zNet;
                break;
            }
        ...
        if (zNetwork == null)
        {
            shell.WriteError($"No station with ClassicStationZLevelsComponent  found in map {mapProto.ID}");
            return;
        }
