...
using Content.Shared._Classic.ZLevels.Mapping.Prototypes;
...
public sealed partial class ClassicMappingZNetworkCommand : LocalizedEntityCommands
{
    ...
    public override string Description => "Load ClassicZLevelMapPrototype as ZNetwork for mapping";


    public override CompletionResult GetCompletion(IConsoleShell shell, string[] args)
    {
        var options = new List<CompletionOption>();
        foreach (var map in _proto.EnumeratePrototypes<ClassicZLevelMapPrototype>())
        {
            options.Add(new CompletionOption(map.ID));
        }

        return CompletionResult.FromHintOptions(options, "ClassicZLevelMapPrototype");
    }
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        //Get Map Prototype
        if (!_proto.Resolve<ClassicZLevelMapPrototype>(args[0], out var indexedZMap))
        {
            shell.WriteError($"Unknown ClassicZLevelMapPrototype {args[0]}");
            return;
        }
