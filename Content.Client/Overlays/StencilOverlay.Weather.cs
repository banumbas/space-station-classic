...
public sealed partial class StencilOverlay
{
        ...
        Matrix3x2 invMatrix)
    {
        ...
        var worldAABB = args.WorldAABB.Enlarged(1f); //Classic: Enlarged(1), because ignoreEmpty disabled, and that cause borderscreen weather flickering
        var worldBounds = args.WorldBounds;
        var position = args.Viewport.Eye?.Position.Position ?? Vector2.Zero;
        var eye = args.Viewport.Eye; //Classic: we need Eye for calculation of isometric wall offset direction
        ...
        worldHandle.RenderInRenderTarget(res.Blep!,
            () =>
            {
                ...
                foreach (var grid in _grids)
                {
                    ...
                    foreach (var tile in _map.GetTilesIntersecting(grid.Owner, grid, worldAABB, ignoreEmpty: false)) //Classic: ignoreEmpty: false, because we can have empty tiles under zLevel roof
                    {
                        // Ignored tiles for stencil
                        if (_weather.CanWeatherAffect((grid.Owner, grid, roofComp), tile))
                            continue;

                        //Classic offset - required for isometric walls
                        if (eye is not null)
                        {
