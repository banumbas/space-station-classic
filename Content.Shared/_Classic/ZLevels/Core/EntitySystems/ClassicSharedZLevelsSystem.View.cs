...
public abstract partial class ClassicSharedZLevelsSystem
{
    ...
    {
        SubscribeLocalEvent<ClassicZLevelViewerComponent, ToggleActionEvent>(OnToggleLookUp);
    }

    private void OnToggleLookUp(Entity<ClassicZLevelViewerComponent> entity, ref ToggleActionEvent args)
    {
        if (args.Handled)
            return;
    ...
    /// </summary>
    public int GetVisibleZLevelsAbove(EntityUid ent, Entity<ClassicZMapComponent?>? currentMapUid = null)
    {
    ...
    /// </summary>
    [PublicAPI]
    public bool HasOpaqueAbove(Vector2 worldPos, Entity<ClassicZMapComponent?> currentMap)
    {
        ...
        float maxSourceDistanceFromOpeningEdgeTiles = float.PositiveInfinity)
    {
        ...
        {
            if (_map.TryGetTileRef(openingMap, grid, tile, out var tileRef) && !ClassicZLevelOpeningCache.IsOpeningTile(tileRef.Tile, TilDefMan))
                return false;
