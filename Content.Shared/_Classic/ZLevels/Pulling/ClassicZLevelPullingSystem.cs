...
namespace Content.Shared._Classic.ZLevels.Pulling;

public sealed partial class ClassicZLevelPullingSystem : EntitySystem
{
    ...
    private EntityQuery<ClassicZLevelPullingTransitionComponent> _transitionQuery;
    private EntityQuery<ClassicZPhysicsComponent> _zPhysicsQuery;

    public override void Initialize()
    {
        base.Initialize();

        _transitionQuery = GetEntityQuery<ClassicZLevelPullingTransitionComponent>();
        _zPhysicsQuery = GetEntityQuery<ClassicZPhysicsComponent>();

        SubscribeLocalEvent<ActivePullerComponent, ClassicZLevelBeforeMapMoveEvent>(OnPullerMove);
        SubscribeLocalEvent<ClassicZLevelPullingTransitionComponent, ClassicZLevelMapMoveEvent>(OnPulledEntityMove);
    }
    ...
    public override void Update(float frameTime)
    {
        ...
        var query = EntityQueryEnumerator<ClassicZLevelPullingTransitionComponent>();
    ...
    private void OnPullerMove(Entity<ActivePullerComponent> ent, ref ClassicZLevelBeforeMapMoveEvent args)
    {
        if (!_timing.IsFirstTimePredicted)
            return;
        ...
        var transComp = EnsureComp<ClassicZLevelPullingTransitionComponent>(pulledEntity);
    ...
    private void OnPulledEntityMove(Entity<ClassicZLevelPullingTransitionComponent> ent, ref ClassicZLevelMapMoveEvent args)
    {
        if (!_timing.IsFirstTimePredicted)
            return;
    ...
    private void FinishTransition(EntityUid uid, ClassicZLevelPullingTransitionComponent comp)
    {
    ...
    /// </summary>
    private void TryResumePulling(EntityUid uid, ClassicZLevelPullingTransitionComponent comp)
    {
        // Check if puller still exists
        if (!Exists(comp.TargetPuller))
        {
            RemComp<ClassicZLevelPullingTransitionComponent>(uid);
            return;
        }
