...
public sealed partial class GridVehicleMoverSystem : EntitySystem
{
    ...
    [Dependency] private readonly VehicleMobSizeSystem _size = default!;
    ...
    private void OnMoverPreventCollide(Entity<GridVehicleMoverComponent> ent, ref PreventCollideEvent args)
    {
        ...
        if (!TryComp(ent.Owner, out ClassicVehicleComponent? vehicle) || vehicle.MovementKind != ClassicVehicleMovementKind.Grid)
            return;
    ...
    public override void Update(float frameTime)
    {
        ...
        var q = EntityQueryEnumerator<GridVehicleMoverComponent, ClassicVehicleComponent, TransformComponent>();

        while (q.MoveNext(out var uid, out var mover, out var vehicle, out var xform))
        {
            if (vehicle.MovementKind != ClassicVehicleMovementKind.Grid)
                continue;
    ...
    private bool ShouldPredictVehicleMovement(ClassicVehicleComponent vehicle)
    {
        if (!_net.IsClient)
            return true;
