...
public sealed class VehicleAmmoLoaderSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<ActorComponent, DidEquipHandEvent>(OnHandItemChanged);
        SubscribeLocalEvent<ActorComponent, DidUnequipHandEvent>(OnHandItemChanged);
    ...
    private void OnHandItemChanged(Entity<ActorComponent> ent, ref DidEquipHandEvent args)
    {
        UpdateOpenLoaderUis(args.User);
    }

    private void OnHandItemChanged(Entity<ActorComponent> ent, ref DidUnequipHandEvent args)
    {
        UpdateOpenLoaderUis(args.User);
    }
    ...
    private void UpdateUi(EntityUid loader, EntityUid user)
    {
        ...
        if (TryGetActiveAmmoBox(loader, user, out var ammoUid))
        {
            ...
            else if (TryComp(ammoUid, out VehicleFlamerTankComponent? heldTankComp) &&
        ...
        foreach (var provider in flamerProviders)
        {
            ...
            for (var i = 0; i < totalSlots; i++)
            {
                ...
                if (insertedTank is { } tankUid &&
                    TryComp(tankUid, out VehicleFlamerTankComponent? tankComp) &&
    ...
    {
        return TryComp(uid, out VehicleFlamerTankComponent? _) && !HasComp<VehicleFlamerAmmoProviderComponent>(uid);
    }
