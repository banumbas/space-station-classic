...
public sealed partial class HardpointSystem : EntitySystem
{
    ...
    {
        var vehicle = owner;
        if (!HasComp<ClassicVehicleComponent>(vehicle) && !TryGetContainingVehicleFrame(owner, out vehicle))
            return;
    ...
    private void OnVehicleCanRun(Entity<HardpointSlotsComponent> ent, ref ClassicVehicleCanRunEvent args)
    {
    ...
    {
        if (!TryComp<ClassicVehicleComponent>(uid, out var vehicle))
            return;
    ...
    private void OnHardpointExamined(Entity<HardpointIntegrityComponent> ent, ref ExaminedEvent args)
    {
        ...
        if (TryComp(ent.Owner, out ClassicVehicleComponent? _) &&
    ...
    private void OnHardpointRepair(Entity<HardpointIntegrityComponent> ent, ref InteractUsingEvent args)
    {
        ...
        var usedWelder = _tool.HasQuality(used, ent.Comp.RepairToolQuality);
        ...
        if (usedWelder && !TryUseWelderFuel(used, args.User, GetFuelCostForChunk(ent.Owner, ent.Comp, repairAmount, isFrame).Float(), true))
        {
            args.Handled = true;
            return;
        }
    ...
    private void OnHardpointRepairDoAfter(Entity<HardpointIntegrityComponent> ent, ref HardpointRepairDoAfterEvent args)
    {
        ...
        var used = args.Used;
        var isFrame = HasComp<HardpointSlotsComponent>(ent.Owner) && !HasComp<HardpointItemComponent>(ent.Owner);
        var usedWelder = used != null && _tool.HasQuality(used.Value, ent.Comp.RepairToolQuality);
        ...
        if (usedWelder)
        {
            if (used == null || !TryUseWelderFuel(used.Value, args.User, GetFuelCostForChunk(ent.Owner, ent.Comp, GetRepairAmountForCurrentStep(ent.Owner, ent.Comp, usedWelder, usedWrench, isFrame), isFrame).Float(), false))
                return;
        }
        ...
        var vehicle = HasComp<ClassicVehicleComponent>(ent.Owner) ? ent.Owner : (GetVehicleFromPart(ent.Owner) ?? ent.Owner);
    ...
    private bool TryUseWelderFuel(EntityUid? used, EntityUid user, float fuel, bool quiet)
    {
        return used != null;
    }
