...
public sealed class ClassicVehicleSystem : EntitySystem
{
    private static readonly EntProtoId VehicleKey = "VehicleKey";
    ...
    [Dependency] private readonly ClassicVehicleBaseSystem _vehicles = default!;
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<VehicleEnterComponent, InteractHandEvent>(OnVehicleEnterInteractHand);
        ...
        SubscribeLocalEvent<VehicleExitComponent, InteractHandEvent>(OnVehicleExitInteractHand);
        ...
        SubscribeLocalEvent<ClassicVehicleOperatorComponent, OnClassicVehicleEnteredEvent>(OnVehicleOperatorEntered);
        SubscribeLocalEvent<ClassicVehicleOperatorComponent, OnClassicVehicleExitedEvent>(OnVehicleOperatorExited);
        ...
        SubscribeLocalEvent<HardpointIntegrityComponent, ClassicVehicleCanRunEvent>(OnFrameVehicleCanRun);
    }
    ...
    private void OnVehicleEnterInteractHand(Entity<VehicleEnterComponent> ent, ref InteractHandEvent args)
    {
        if (args.Handled)
            return;

        var activateArgs = new ActivateInWorldEvent(args.User, ent.Owner, false);
        OnVehicleEnterActivate(ent, ref activateArgs);
        args.Handled = activateArgs.Handled;
    }
    ...
    private void CleanupInterior(EntityUid vehicle)
    {
        ...
        {
            _map.QueueDeleteMap(interior.MapId);
        }
    ...
    private void OnVehicleExitInteractHand(Entity<VehicleExitComponent> ent, ref InteractHandEvent args)
    {
        if (args.Handled)
            return;

        var activateArgs = new ActivateInWorldEvent(args.User, ent.Owner, false);
        OnVehicleExitActivate(ent, ref activateArgs);
        args.Handled = activateArgs.Handled;
    }
    ...
    private bool TryExit(Entity<VehicleExitComponent> ent, EntityUid user)
    {
        ...
        _transform.SetMapCoordinates(user, exitMapCoords);
    ...
    private void OnDriverSeatStrapped(Entity<VehicleDriverSeatComponent> ent, ref StrappedEvent args)
    {
        ...
        if (!TryGetVehicleFromInterior(ent.Owner, out var vehicle) ||
            !TryComp(vehicle, out ClassicVehicleComponent? vehicleComp))
        {
            return;
        }
    ...
    private void OnDriverSeatUnstrapped(Entity<VehicleDriverSeatComponent> ent, ref UnstrappedEvent args)
    {
        ...
        {
            RemCompDeferred<ClassicVehicleOperatorComponent>(args.Buckle.Owner);
        }
    }

    private void OnVehicleOperatorEntered(Entity<ClassicVehicleOperatorComponent> ent, ref OnClassicVehicleEnteredEvent args)
    {
        if (_net.IsClient)
            return;
    ...
    private void OnVehicleOperatorExited(Entity<ClassicVehicleOperatorComponent> ent, ref OnClassicVehicleExitedEvent args)
    {
        if (_net.IsClient)
            return;
    ...
    private void OnFrameVehicleCanRun(Entity<HardpointIntegrityComponent> ent, ref ClassicVehicleCanRunEvent args)
    {
    ...
    public bool TryResolveControlledVehicle(EntityUid user, out EntityUid vehicle)
    {
        ...
        if (TryComp<ClassicVehicleOperatorComponent>(user, out var op) &&
    ...
    public bool TryGetInteriorPeekTarget(EntityUid vehicle, int entryIndex, out EntityUid target)
    {
        ...
        if (!TryComp(vehicle, out VehicleEnterComponent? _) ||
