...
public sealed class VehicleDeploySystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<VehicleDeployableComponent, ClassicVehicleCanRunEvent>(OnVehicleCanRun);
    ...
    private void OnDeployAction(Entity<VehicleDeployActionComponent> ent, ref VehicleDeployActionEvent args)
    {
        ...
        if (!TryComp(vehicle, out ClassicVehicleComponent? vehicleComp) ||
    ...
    private void OnVehicleCanRun(Entity<VehicleDeployableComponent> ent, ref ClassicVehicleCanRunEvent args)
    {
        if (!args.CanRun)
            return;
    ...
    {
        if (!TryComp(vehicle, out ClassicVehicleComponent? vehicleComp) || vehicleComp.Operator == null)
            return;
    ...
    private bool IsValidAutoTarget(EntityUid vehicle, VehicleDeployableComponent deployable, EntityUid target, float range)
    {
        ...
        return (vehiclePos - targetPos).LengthSquared() <= range * range;
    }
    ...
    {
        _popup.PopupEntity(message, vehicle, deployer);
    }
