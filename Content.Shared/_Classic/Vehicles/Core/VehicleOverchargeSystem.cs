...
public sealed class VehicleOverchargeSystem : EntitySystem
{
    ...
    [Dependency] private readonly ClassicVehicleSystem _vehicles = default!;
    ...
    public override void Initialize()
    {
        if (_net.IsClient)
        {
            CommandBinds.Builder
                .Bind(VehicleKeyFunctions.VehicleUniqueAction, InputCmdHandler.FromDelegate(session =>

                {
                    if (session?.AttachedEntity is not { } user)
                        return;

                    if (!_vehicles.TryResolveControlledVehicle(user, out var vehicleUid))
                        return;
    ...
    private void OnOverchargeRequest(VehicleOverchargeRequestEvent ev, EntitySessionEventArgs args)
    {
        ...
        if (!TryComp(vehicle, out ClassicVehicleComponent? vehicleComp) || vehicleComp.Operator != user)
            return;
