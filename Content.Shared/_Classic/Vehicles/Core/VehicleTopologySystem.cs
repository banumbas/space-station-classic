...
public readonly record struct VehicleMountedFlamerProvider(
    VehicleMountedSlot Slot,
    EntityUid FlamerUid,
    VehicleFlamerAmmoProviderComponent FlamerAmmo,
    int ExtraSlots);
...
public sealed class VehicleTopologySystem : EntitySystem
{
    ...
    {
        return TryGetContainerAncestor<ClassicVehicleComponent>(uid, out vehicle, includeSelf);
    }
        ...
        out VehicleMountedFlamerProvider provider)
    {
        ...
        if (!TryComp(item, out VehicleFlamerAmmoProviderComponent? flamerAmmo))
            return false;

        if (HasComp<VehicleFlamerTankComponent>(item))
            return false;
