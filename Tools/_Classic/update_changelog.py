...
def update_changelog():
    ...
    if ":cl:" in cleaned_body:
        ...
        def datetime_representer(dumper, data):
            # Output in ISO format with 'T' and microseconds
            return dumper.represent_scalar('tag:yaml.org,2002:timestamp', data.isoformat(timespec='microseconds'))

        yaml.add_representer(datetime, datetime_representer)
        yaml.add_representer(datetime, datetime_representer, Dumper=yaml.SafeDumper)

        with open(changelog_path, "w", encoding='utf-8') as file:
            yaml.dump(changelog_data, file, allow_unicode=True, default_flow_style=False)
            file.write('\n')
